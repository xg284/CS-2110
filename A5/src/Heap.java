...
public class Heap<E> {
    ...
    public void add(E v, double p) throws IllegalArgumentException {
        ...
        if (map.get(v) != null) { throw new IllegalArgumentException("v is already in the heap"); }
        Element res= new Element(v, p);
        ensureCapacity();
        size++ ;
        b[size - 1]= res;
        map.put(v, size - 1);
    ...
    /** If size = length of arry b, double the length of array b. <br>
    ...
    public void updatePriority(E v, double p) {
        ...
        if (map.get(v) == null) throw new IllegalArgumentException("v is not in the heap");
        int k= map.get(v);
        b[k].priority= p;
        bubbleUp(k);
        bubbleDown(k);
